...
import model.dao.NcmDAO;
import util.Util;


public class ViewNcm extends javax.swing.JInternalFrame {
 
   
    public ViewNcm() {
        initComponents();
        DefaultTableModel modelo = (DefaultTableModel) TabelaNcm.getModel();
        TabelaNcm.setRowSorter(new TableRowSorter(modelo));
        readTable();
    ...
    private void initComponents() {
        ...
        TabelaNcm = new javax.swing.JTable();
        ...
        setClosable(true);
        setTitle("ENTERPRISE - CADASTRO DA NOMECLATURA COMUM DO MERCOSUL");

        TabelaNcm.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.LOWERED));
        TabelaNcm.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "ID NCM", "DESCRIÇÃO", "NCM", "CADASTRADO POR", "DT CADASTRO", "ALTERADO POR", "DT ALTERAÇÃO", "CANCELADO"
            }
        ) {
        ...
        TabelaNcm.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        TabelaNcm.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                TabelaNcmMouseClicked(evt);
            }
        });
        TabelaNcm.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                TabelaNcmKeyReleased(evt);
            }
        });
        ...
        lbldescricao.setText("DESCRIÇÃO NCM");
        ...
        lblncm.setText("NCM - APENAS 08 DIGITOS ");
                ...
                .addGroup(PainelDadosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(PainelDadosLayout.createSequentialGroup()
                        ...
                        .addGroup(PainelDadosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(PainelDadosLayout.createSequentialGroup()
                                .addGap(10, 10, 10)
                                .addComponent(txtncm, javax.swing.GroupLayout.PREFERRED_SIZE, 112, javax.swing.GroupLayout.PREFERRED_SIZE)))))
    ...
    private void TabelaNcmKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TabelaNcmKeyReleased
        passagemderegistros();
    }//GEN-LAST:event_TabelaNcmKeyReleased
    ...
    private void txtncmKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtncmKeyPressed
         if(evt.getKeyCode() == evt.VK_ENTER){
            if (txtncm.getText().trim().isEmpty()){
                JOptionPane.showMessageDialog(null, "CAMPO NCM É OBRIGATÓRIO !", "ENTERPRISE - ATENÇÃO", JOptionPane.ERROR_MESSAGE);
                txtncm.requestFocus();
            }else {
                 if(txtncm.getText().trim().length() != 8) {
                    JOptionPane.showMessageDialog(null, "CAMPO NCM DEVERÁ POSSUIR 8 (OITO) NÚMEROS OBRIGATÓRIAMENTE !", "ENTERPRISE - ATENÇÃO", JOptionPane.ERROR_MESSAGE);
                    txtncm.requestFocus();
                 }else{
                     btnincluir.requestFocus();
                 }        
    ...
    public void readTable() {
        ...
        for (Ncm nc : ncmdao.read()) {
            modelo.addRow(new Object[]{
                nc.getId_ncm(),
                nc.getDescricao(),
                nc.getNcm(),
                nc.getUsuariocadastro(),
                df.format(nc.getDtcadastro()),
                nc.getUsuarioalteracao(),
                df.format(nc.getDtalteracao()),
                nc.getCancelado()
    ...
    public void readTableForDesc(String desc) {        
        ...
        NcmDAO ncmdao = new NcmDAO();
        DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
        for(Ncm nc: ncmdao.readForDesc(desc)){
            modelo.addRow(new Object[]{                
                nc.getId_ncm(),
                nc.getDescricao(),
                nc.getNcm(),
    ...
    public void incluir() {
        ...
        if (txtdescricao.getText().trim().isEmpty() || txtncm.getText().trim().isEmpty()) {
            txtdescricao.requestFocus();
            JOptionPane.showMessageDialog(null, "CAMPOS DESCRICÃO / NCM SÃO OBRIGATÓRIOS !", "ENTERPRISE - ATENÇÃO", JOptionPane.ERROR_MESSAGE);
        } else {
            int resposta = JOptionPane.showConfirmDialog(null, "CONFIRMA A INCLUSÃO ?", "ENTERPRISE - DECISÃO", JOptionPane.YES_NO_OPTION);
            if (resposta == JOptionPane.YES_OPTION) {
                if (ncmdao.pesquisancm(txtncm.getText()) == null) {
                    nc.setNcm(txtncm.getText().toUpperCase());
                    nc.setDescricao(txtdescricao.getText().toUpperCase());
                    nc.setUsuariocadastro(Integer.parseInt(ViewLogin.Id_Colaborador));
                    nc.setDtcadastro(Util.formatarDataPostgres(dataSistema));
                    nc.setUsuarioalteracao(Integer.parseInt(ViewLogin.Id_Colaborador));
                    nc.setDtalteracao(Util.formatarDataPostgres(dataSistema));
                    nc.setCancelado("N");
                    //ncmdao.create(nc);
                    NcmDAO.getInstance().salvar(nc);
                    readTable();
                } else {
                    JOptionPane.showMessageDialog(null, "NCM JÁ EXISTE EFETUE A MUDANÇA !", "ENTERPRISE - ATENÇÃO", JOptionPane.ERROR_MESSAGE);
                    txtncm.requestFocus();
                }
            }
        }
    ...
    public void atualizar() {
        if (TabelaNcm.getSelectedRow() != -1) {
            ...
            if (resposta == JOptionPane.YES_OPTION) {
                if (ncmdao.pesquisancm(txtncm.getText()) == null) {
                    nc.setId_ncm((int) TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 0));
                    nc.setNcm(txtncm.getText().toUpperCase());
                    nc.setDescricao(txtdescricao.getText().toUpperCase());
                    nc.setUsuarioalteracao(Integer.parseInt(ViewLogin.Id_Colaborador));
                    nc.setDtalteracao(Util.formatarDataPostgres(dataSistema));
...
//    }
     ...
     public void cancelar() {   // ESTE MÉTODO CANCELA O  REGISTRO DO NCM
        if (TabelaNcm.getSelectedRow() != -1) {
            Date dataSistema = new Date();
            Ncm nc = new Ncm();
            NcmDAO ncmdao = new NcmDAO();
            if (chkcancelado.isSelected()) {
                ...
                if (resposta == JOptionPane.YES_OPTION) {
                    ...
                    chkcancelado.setSelected(false);
                    //ncmdao.cancelar(nc);
                    NcmDAO.getInstance().cancelar(nc);
                }
           } else {
    ...
    public void passagemderegistros() {
        if (TabelaNcm.getSelectedRow() != -1) {
            ...
            txtdescricao.setText(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 1).toString());
            txtncm.setText(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 2).toString());

            if (TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 3).toString().equals("0")) {
                txtusuariocadastro.setText("ADMINISTRADOR");
            } else {
                txtusuariocadastro.setText(colaboradordao.pesquisaId_Colaborador(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 3).toString()));
            }
            txtdtcadastro.setText(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 4).toString());
            if (TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 5).toString().equals("0")) {
                txtusuarioalteracao.setText("ADMINISTRADOR");
            } else {
                txtusuarioalteracao.setText(colaboradordao.pesquisaId_Colaborador(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 5).toString()));
            }
            txtdtalteracao.setText(TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 6).toString());
            if (TabelaNcm.getValueAt(TabelaNcm.getSelectedRow(), 7).toString().equals("S")) {
                chkcancelado.setSelected(true);
            } else {
                chkcancelado.setSelected(false);
            }
    ...
    private javax.swing.JTable TabelaNcm;
